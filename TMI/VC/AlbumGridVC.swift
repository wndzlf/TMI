...
class AlbumGridVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    private var albumDictionary: [String:[PHAsset]] = ["kakaoTalk":[], "daumCafe": [], "instagram":[], "others":[]]
...
//    private var assetLocalIdentifier: [String] = []
    ...
    fileprivate func setCollectionView(_ defaults: UserDefaults) {
        ...
        switch photoAuthorizationStatus {
            ...
            case .notDetermined: //허가하는지 안하는지 선택하지 않으면
                ...
                PHPhotoLibrary.requestAuthorization({ (status) in //다시 허가 요청
                    switch status {
                    case .authorized:
                        self.setCollectionView(defaults)
                    case .denied:
                        print("사용자가 불허함")
                    default: break
                    }
